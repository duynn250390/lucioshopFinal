	...
	<div class="header_content">
		...
		<div class="info_bds ">
			...
			<span class="info">
				...
				<span class="val_gia">
					<?php $quan =  get_post_meta( get_the_ID(), '_quan', TRUE ); 
					if(empty($quan)){
						echo 'Đang cập nhật';
					}else{
						echo $quan ;
				...
				<div class="box_colum">
					<div class="row_box">
						...
						<div class="right">
							...
							if(count($category)>0){
								$name_loaitin = $category[0]->cat_name;
								echo $name_loaitin ;
							}else{
								echo 'Đang cập nhật';
							}
					...
					<div class="row_box">
						...
						<div class="right"><?php echo get_post_meta( get_the_ID(), '_huong_nha', TRUE ); ?></div>
					</div>
				</div>
			</div>
		</div>
...
<div class="news_more">
	<h2 class="heading_more"><span>Bất động sản liên quan</span></h2>
	<div class="box_conent_news">
		<?php
		$thuoc_du_an =  get_post_meta( get_the_ID(), '_thuoc_du_an', TRUE ); 
		if(!empty($thuoc_du_an)){
			$query_args = array(
				'post_type' => $post_type,
				"taxonomy" => $thuoc_du_an,
				'post_status' => 'publish',
				'posts_per_page' => 5,
			);
					...
					<div class="main_content_news_item">
						<div class="title_content_top">
							...
							<div class="info_content_bottom">
								<div>
									<div class="left space_left">Giá</div>: <?php echo $gia_tien ?> (<?php echo $tinh_trang ?>)
								</div>
								<div>
									<div class="left space_left">Diện tích</div>: <?php echo $dien_tich;?>m<sup>2</sup>
								</div>
								<div>
									<div class="left space_left">Hướng</div>: <?php echo $huong ?>
								</div>
							</div>
						</div>
					</div>
