...
function add_gallery_rend_home_metabox($post_type) {
	$types = array('rend_home');
	if (in_array($post_type, $types)) {
		add_meta_box(
			'gallery-metabox',
			'Thêm hình ảnh',
			'gallery_rend_home_meta_callback',
			$post_type,
			'normal',
			'high'
		);
...
add_action('add_meta_boxes', 'add_gallery_rend_home_metabox');

function gallery_rend_home_meta_callback($post) {
	wp_nonce_field( basename(__FILE__), 'gallery_meta_nonce' );
	$ids = get_post_meta($post->ID, 'tdc_gallery_id', true);
	?>
	<table class="form-table">
		<tr><td>
			<a class="gallery-add button" href="#" data-uploader-title="Thêm hình ảnh" data-uploader-button-text="Thêm nhiều hình ảnh">Thêm hình ảnh nhà cho thuê</a>
	...
	<?php }
	function gallery_rend_home_meta_save($post_id) {
	...
	add_action('save_post', 'gallery_rend_home_meta_save');

	function gallery_rend_home_enqueue_hook($hook) {
	...
	add_action('admin_enqueue_scripts', 'gallery_rend_home_enqueue_hook');

// Thông tin người nhập và liên hệ
// =======================================
	function info_rend_home_meta_box(){
		$id = 'info_home_meta_box';
		$title = 'Thông tin nhà';
		$callback = 'info_rend_home_output';
		$screen = 'rend_home';
		add_meta_box( $id, $title, $callback,$screen);

	}
	add_action('add_meta_boxes','info_rend_home_meta_box');
	function info_rend_home_output($post){
		...
		$duong_truoc_nha = get_post_meta( $post->ID, '_duong_truoc_nha', true );
		...
		$phong_khach = get_post_meta( $post->ID, '_phong_khach', true );
		$tang = get_post_meta( $post->ID, '_tang', true );
		$phong_ngu = get_post_meta( $post->ID, '_phong_ngu', true );
		$phong_tam = get_post_meta( $post->ID, '_phong_tam', true );
		?>
		<style type="text/css">
	...
	<div class="box_info_meta_box">
		<div class="item_list">
			<div class="item_list_box">
				...
				<select  name="huong_nha" id="huong_nha" class="code">
					...
					<option value="Không xác định" <?php if($huong_nha == 'Không xác định'){echo 'selected';} ?>>Không xác định</option>
				</select>
			</div>	
		</div>	
		<div class="item_list">
			<div class="item_list_box">
				<lable for="duong_truoc_nha" style="display: block;">Đường trước nhà(m)</lable>
				<input name="duong_truoc_nha" class="code"  id="duong_truoc_nha" type="text" value="<?php echo $duong_truoc_nha ?>">
			</div>	
		</div>	
		<div class="item_list">
			<div class="item_list_box">
				<lable for="phong_ngu" style="display: block;">Số phòng ngủ</lable>
				<select  name="phong_ngu" id="phong_ngu" class="code">
					<?php
					$phong = 10;
					for ($i=1; $i <= $phong; $i++) { ?>
					<option value='<?php echo $i?>'  <?php if($phong_ngu == $i) echo 'selected'?>><?php echo $i?> phòng</option>
					<?php }?>
					<option value='Không xác định' <?php if($phong_ngu == 'Không xác định') echo 'selected'?>>Không xác định</option>
				</select>
			</div>	
		</div>	
		<div class="item_list">
			<div class="item_list_box">
				<lable for="phong_khach" style="display: block;">Phòng khách</lable>
				<select  name="phong_khach" id="phong_khach" class="code">
					<?php
					$phong = 10;
					for ($i=1; $i <= $phong; $i++) { ?>
					<option value='<?php echo $i?>' <?php if($phong_khach == $i) echo 'selected'?>><?php echo $i?> phòng</option>
					<?php }?>
					<option value='Không xác định' <?php if($phong_khach == 'Không xác định') echo 'selected'?>>Không xác định</option>
				</select>
			</div>	
		</div>
		<div class="item_list">
			<div class="item_list_box">
				<lable for="phong_tam" style="display: block;">Phòng tắm</lable>
				<select  name="phong_tam" id="phong_tam" class="code">
					<?php
					$phong = 10;
					for ($i=1; $i <= $phong; $i++) { ?>
					<option value='<?php echo $i?>' <?php if($phong_tam == $i) echo 'selected'?>><?php echo $i?> phòng</option>
					<?php }?>
					<option value='Không xác định' <?php if($phong_tam == 'Không xác định') echo 'selected'?>>Không xác định</option>
				</select>
			</div>	
		</div>
		<div class="item_list">
			<div class="item_list_box">
				<lable for="tang" style="display: block;">Số tầng</lable>
				<select  name="tang" id="tang" class="code">
					<?php
					$floot = 10;
					for ($i=1; $i <= $floot; $i++) { ?>
					<option value='<?php echo $i?>' <?php if($tang == $i) echo 'selected'?>><?php echo $i?> tầng</option>
					<?php }?>
					<option value='Không xác định' <?php if($tang == 'Không xác định') echo 'selected'?>>Không xác định</option>
				</select>
			</div>	
		</div>	
		...
		<div class="item_list">
			<div class="item_list_box">
				<lable for="dien_tich" style="display: block;">Diện tích sử dụng(m<sup>2</sup>)</lable>
				<input name="dien_tich" class="code"  id="dien_tich" type="text" value="<?php echo $dien_tich ?>">
			</div>	
		</div>	
		<div class="item_list">
			<div class="item_list_box">
				<lable for="gia_tien" style="display: block;">Giá tiền(VNĐ/ tháng)</lable>
		...
		<div class="item_list">
			<div class="item_list_box">
				...
				<?php }else{ ?>
				<select  name="loai_tin" id="loai_tin" class="code">
					<option value="1" <?php if($loai_tin == 1){echo 'selected';} ?>>Thường</option>
				</select>	
				<?php } ?>
			</div>		
		</div>		
...
function info_rend_home_save($post_id){
	...
	if(isset($_POST['duong_truoc_nha'])){
		$duong_truoc_nha = sanitize_text_field( $_POST['duong_truoc_nha'] );
		update_post_meta($post_id,'_duong_truoc_nha',$duong_truoc_nha);
	}
	...
	if(isset($_POST['tinh_thanhpho'])){
		$tinh_thanhpho= sanitize_text_field( $_POST['tinh_thanhpho'] );
		update_post_meta($post_id,'_tinh_thanhpho',$tinh_thanhpho);
	}
	if(isset($_POST['quan'])){
	...
	if(isset($_POST['phuong_xa'])){
		...
		update_post_meta($post_id,'_phuong_xa',$phuong_xa);
	}	
	if(isset($_POST['phong_tam'])){
		$phong_tam= sanitize_text_field( $_POST['phong_tam'] );
		update_post_meta($post_id,'_phong_tam',$phong_tam);
	}
	if(isset($_POST['phong_ngu'])){
		$phong_ngu= sanitize_text_field( $_POST['phong_ngu'] );
		update_post_meta($post_id,'_phong_ngu',$phong_ngu);
	}
	if(isset($_POST['phong_khach'])){
		$phong_khach= sanitize_text_field( $_POST['phong_khach'] );
		update_post_meta($post_id,'_phong_khach',$phong_khach);
	}
	if(isset($_POST['tang'])){
		$tang= sanitize_text_field( $_POST['tang'] );
		update_post_meta($post_id,'_tang',$tang);
	}
	if(isset($_POST['info_dia_chi'])){
		$info_dia_chi= sanitize_text_field( $_POST['info_dia_chi'] );
		update_post_meta($post_id,'_info_dia_chi',$info_dia_chi);
	}
}
add_action('save_post','info_rend_home_save');

// Thông tin người nhập và liên hệ

function info_auth_rend_home_meta_box_home(){
	$id = 'info_auth_meta_box_home';
	$title = 'Thông tin người đăng';
	$callback = 'info_auth_rend_home_output_home';
	$screen = 'rend_home';
	add_meta_box( $id, $title, $callback,$screen);

}
add_action('add_meta_boxes','info_auth_rend_home_meta_box_home');

function info_auth_rend_home_output_home($post){
	global $current_user;
...
function info_auth_save_rend_home($post_id){
...
add_action('save_post','info_auth_save_rend_home');
